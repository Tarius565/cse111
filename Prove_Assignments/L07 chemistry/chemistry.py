...
author: Tanner Levi
purpose: write and return values inside a compounded list
"""

def main():
    # Get a chemical formula for a molecule from the user.
    formula = input("Enter a chemical formula for a molecule: ")
    # Get the mass of a chemical sample in grams from the user.
    mass_sample = float(input("Enter the mass of the chemical sample (g): "))
    # Call the make_periodic_table function and
    # store the periodic table in a variable.
    periodic_list = make_periodic_table()
    # Stretch goal. find the name of the formula
    formula_name_list = make_known_molecules()
    formula_name = get_formula_name(formula, formula_name_list)
    ...
    # Stretch goal. Print the formula name.
...
# def main():
#     #Call make_periodic_table function to print a name and atomic mass
#     periodic_list = []
#     periodic_list = make_periodic_table()
#     print(periodic_list)
#     for i in range(len(periodic_list)):
#         print(f"{periodic_list[i][1]} {periodic_list[i][2]}")
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for i in range(len(symbol_quantity_list)):
        symbol = symbol_quantity_list[i][SYMBOL_INDEX]
        qty = symbol_quantity_list[i][QUANTITY_INDEX]
        atomic_mass = periodic_table_dict[symbol][ATOMIC_MASS_INDEX]
        molar_mass = molar_mass + atomic_mass * qty
